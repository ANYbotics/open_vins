...
void TrackAruco::display_active(cv::Mat &img_out, int r1, int g1, int b1, int r2, int g2, int b2) {
    ...
    for(auto const& pair : img_last_cache) {
        ...
        if(!rejects[pair.first].empty()) cv::aruco::drawDetectedMarkers(img_temp, rejects[pair.first], cv::noArray(), cv::Scalar(b1,g1,r1));
        // Draw what camera this is
        auto txtpt = (is_small)? cv::Point(10,30) : cv::Point(30,60);
        cv::putText(img_temp, "CAM:"+std::to_string((int)pair.first), txtpt, cv::FONT_HERSHEY_COMPLEX_SMALL, (is_small) ? 1.5 : 3.0, cv::Scalar(b2,g2,r2), 3);
